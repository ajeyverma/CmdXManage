...
Show-Header
    ...
    Write-Host "6. Add All Linux Commands"
    write-Host "7. Exit"    
}
...
function Add-AllLinuxCommands {
    ...
    Write-Host ""
    Write-Host "Adding Linux-style commands to your profile..." -ForegroundColor Cyan
    Write-Host ""

    # Define Linux commands with their PowerShell equivalents
    $linuxCommands = @{
        # Basic file operations
        'll' = 'Get-ChildItem -Force @args'
        'la' = 'Get-ChildItem -Force -Hidden @args'
        'ls' = 'Get-ChildItem @args'
        'cat' = 'Get-Content @args'
        'grep' = 'Select-String @args'
        'find' = 'Get-ChildItem -Recurse -Filter @args'
        'rm' = 'Remove-Item -Force @args'
        'rmdir' = 'Remove-Item -Directory @args'
        'cp' = 'Copy-Item @args'
        'mv' = 'Move-Item @args'
        'pwd' = 'Get-Location'
        'touch' = 'if ($args) { foreach ($file in $args) { if (-not (Test-Path $file)) { New-Item -ItemType File -Path $file -Force | Out-Null } else { (Get-Item $file).LastWriteTime = Get-Date } } } else { Write-Host "Usage: touch <file>" }'
        'head' = 'Get-Content @args -Head 10'
        'tail' = 'Get-Content @args -Tail 10'
        'less' = 'Get-Content @args | more'
        'more' = 'Get-Content @args | more'
        'wc' = '$lines = (Get-Content @args | Measure-Object -Line).Lines; $words = (Get-Content @args | Measure-Object -Word).Words; $chars = (Get-Content @args | Measure-Object -Character).Characters; Write-Host "$lines $words $chars"'
        
        # System information
        'which' = 'Get-Command @args | Select-Object -ExpandProperty Source'
        'ps' = 'Get-Process @args'
        'kill' = 'Stop-Process -Id @args'
        'killall' = 'Get-Process @args | Stop-Process -Force'
        'top' = 'Get-Process | Sort-Object CPU -Descending | Select-Object -First 20 | Format-Table -AutoSize'
        'clear' = 'Clear-Host'
        'history' = 'Get-History @args'
        'alias' = 'Get-Alias @args'
        'echo' = 'Write-Output @args'
        'whoami' = 'Write-Output $env:USERNAME'
        'id' = 'Write-Output "uid=$($env:USERNAME) gid=$($env:USERNAME) groups=$($env:USERNAME)"'
        'uname' = 'if ($args -contains "-a") { Get-CimInstance Win32_OperatingSystem | Select-Object Caption, Version, OSArchitecture | Format-List } else { Write-Output (Get-CimInstance Win32_OperatingSystem).Caption }'
        'date' = 'Get-Date'
        'cal' = '$month = if ($args) { [int]$args[0] } else { (Get-Date).Month }; $year = if ($args.Length -gt 1) { [int]$args[1] } else { (Get-Date).Year }; $firstDay = Get-Date -Year $year -Month $month -Day 1; $lastDay = $firstDay.AddMonths(1).AddDays(-1); Write-Host "`n   $(Get-Culture).DateTimeFormat.GetMonthName($month) $year`n"; Write-Host "Su Mo Tu We Th Fr Sa"; $dayOfWeek = [int]$firstDay.DayOfWeek; Write-Host ("   " * $dayOfWeek) -NoNewline; for ($day = 1; $day -le $lastDay.Day; $day++) { Write-Host ("{0,2} " -f $day) -NoNewline; if (([int]$firstDay.AddDays($day-1).DayOfWeek) -eq 6) { Write-Host "" } }; Write-Host "`n"'
        'df' = 'Get-PSDrive -PSProvider FileSystem | Select-Object Name, @{Name="Size(GB)";Expression={[math]::Round($_.Used/1GB,2)}}, @{Name="Used(GB)";Expression={[math]::Round($_.Used/1GB,2)}}, @{Name="Avail(GB)";Expression={[math]::Round(($_.Free)/1GB,2)}}, @{Name="Use%";Expression={[math]::Round(($_.Used/($_.Used+$_.Free))*100,2)}} | Format-Table -AutoSize'
        'du' = 'if ($args) { Get-ChildItem $args -Recurse -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum | Select-Object @{Name="Size(MB)";Expression={[math]::Round($_.Sum/1MB,2)}} } else { Get-ChildItem . -Recurse -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum | Select-Object @{Name="Size(MB)";Expression={[math]::Round($_.Sum/1MB,2)}} }'
        'free' = 'Get-CimInstance Win32_OperatingSystem | Select-Object @{Name="Total(GB)";Expression={[math]::Round($_.TotalVisibleMemorySize/1MB/1024,2)}}, @{Name="Used(GB)";Expression={[math]::Round(($_.TotalVisibleMemorySize-$_.FreePhysicalMemory)/1MB/1024,2)}}, @{Name="Free(GB)";Expression={[math]::Round($_.FreePhysicalMemory/1MB/1024,2)}} | Format-List'
        
        # Text processing
        'sort' = 'Get-Content @args | Sort-Object'
        'uniq' = 'Get-Content @args | Get-Unique'
        'diff' = 'Compare-Object (Get-Content $args[0]) (Get-Content $args[1])'
        'cut' = 'if ($args -match "-d(.+)\s+-f(.+)") { $delim = $matches[1]; $field = [int]$matches[2]; Get-Content $args[-1] | ForEach-Object { ($_ -split $delim)[$field-1] } } else { Write-Host "Usage: cut -d<delimiter> -f<field> <file>" }'
        'tee' = 'if ($args -match "-a\s+(.+)") { $file = $matches[1]; $input | Tee-Object -FilePath $file -Append } elseif ($args) { $input | Tee-Object -FilePath $args } else { $input | Tee-Object }'
        'xargs' = 'if ($args) { $input | ForEach-Object { & $args[0] $_ $args[1..($args.Length-1)] } } else { Write-Host "Usage: <command> | xargs <command>" }'
        
        # Directory navigation
        'cd' = 'Set-Location @args'
        'cd..' = 'Set-Location ..'
        'cd-' = 'if ($global:OLDPWD) { $current = Get-Location; Set-Location $global:OLDPWD; $global:OLDPWD = $current } else { Write-Host "No previous directory" }'
        'tree' = 'Get-ChildItem -Recurse @args | Select-Object FullName | ForEach-Object { $_.FullName.Replace((Get-Location).Path + "\", "") }'
        'pushd' = 'Push-Location @args; $global:OLDPWD = Get-Location'
        'popd' = 'Pop-Location; $global:OLDPWD = Get-Location'
        
        # Network commands
        'curl' = 'Invoke-WebRequest @args | Select-Object -ExpandProperty Content'
        'wget' = 'Invoke-WebRequest @args -OutFile (Split-Path -Leaf $args[-1])'
        'ping' = 'Test-Connection @args'
        'ifconfig' = 'Get-NetIPAddress | Format-Table -AutoSize'
        'netstat' = 'Get-NetTCPConnection | Format-Table -AutoSize'
        'traceroute' = 'Test-NetConnection @args -TraceRoute'
        
        # File permissions and ownership (Windows equivalents)
        'chmod' = 'Write-Host "chmod: Windows uses ACLs. Use: icacls <file> /grant <user>:<permissions>" -ForegroundColor Yellow'
        'chown' = 'Write-Host "chown: Windows uses ACLs. Use: icacls <file> /setowner <user>" -ForegroundColor Yellow'
        'ln' = 'if ($args[0] -eq "-s") { New-Item -ItemType SymbolicLink -Path $args[2] -Target $args[1] } else { New-Item -ItemType HardLink -Path $args[1] -Target $args[0] }'
        
        # Archive operations
        'tar' = 'if ($args[0] -eq "-xzf") { Expand-Archive -Path $args[1] -DestinationPath (Split-Path $args[1]) } elseif ($args[0] -eq "-czf") { Compress-Archive -Path $args[1] -DestinationPath $args[2] } else { Write-Host "Usage: tar -xzf <file> (extract) or tar -czf <archive> <files> (create)" }'
        'zip' = 'Compress-Archive -Path @args'
        'unzip' = 'Expand-Archive -Path @args'
        
        # Process and job management
        'jobs' = 'Get-Job'
        'fg' = 'if ($args) { Resume-Job $args[0] | Wait-Job | Receive-Job } else { Write-Host "Usage: fg <job_id>" }'
        'bg' = 'if ($args) { Resume-Job $args[0] } else { Write-Host "Usage: bg <job_id>" }'
        'nohup' = 'Start-Process powershell -ArgumentList "-NoProfile -Command", ($args -join " ") -WindowStyle Hidden'
        
        # Environment
        'env' = 'Get-ChildItem Env: | Format-Table -AutoSize'
        'export' = 'if ($args -match "(.+)=(.+)") { [Environment]::SetEnvironmentVariable($matches[1], $matches[2], "User") } else { Write-Host "Usage: export VAR=value" }'
        'source' = '. @args'
        
        # Utilities
        'bc' = 'Write-Host "bc: Use PowerShell calculator: [math]::<operation>" -ForegroundColor Yellow'
        'man' = 'Get-Help @args'
        'help' = 'Get-Help @args'
        'watch' = 'if ($args) { while ($true) { Clear-Host; & $args[0] $args[1..($args.Length-1)]; Start-Sleep -Seconds 2 } } else { Write-Host "Usage: watch <command> [args]" }'
        'time' = 'Measure-Command { & @args }'
        'whereis' = 'Get-Command @args | Select-Object -ExpandProperty Source'
        
        # File comparison and search
        'locate' = 'Get-ChildItem -Recurse -Filter @args -ErrorAction SilentlyContinue | Select-Object FullName'
        'updatedb' = 'Write-Host "updatedb: Not needed on Windows. Use: Get-ChildItem -Recurse" -ForegroundColor Yellow'
        'strings' = 'Get-Content @args | Select-String -Pattern "[^\x00-\x1F]{4,}"'
        
        # System monitoring
        'uptime' = '$os = Get-CimInstance Win32_OperatingSystem; $uptime = (Get-Date) - $os.ConvertToDateTime($os.LastBootUpTime); Write-Host "up $($uptime.Days) days, $($uptime.Hours):$($uptime.Minutes.ToString("00"))"'
        'iostat' = 'Get-Counter "\Processor(_Total)\% Processor Time" | Select-Object -ExpandProperty CounterSamples | Format-Table'
        'vmstat' = 'Get-CimInstance Win32_OperatingSystem | Select-Object @{Name="TotalMem(GB)";Expression={[math]::Round($_.TotalVisibleMemorySize/1MB/1024,2)}}, @{Name="FreeMem(GB)";Expression={[math]::Round($_.FreePhysicalMemory/1MB/1024,2)}}, @{Name="UsedMem(GB)";Expression={[math]::Round(($_.TotalVisibleMemorySize-$_.FreePhysicalMemory)/1MB/1024,2)}}'
        
        # Additional useful commands
        'mkdir' = 'New-Item -ItemType Directory -Path @args'
        'lns' = 'if ($args.Length -ge 2) { New-Item -ItemType SymbolicLink -Path $args[1] -Target $args[0] } else { Write-Host "Usage: lns <target> <link>" }'
        'mount' = 'Get-PSDrive -PSProvider FileSystem | Format-Table'
        'umount' = 'Write-Host "umount: Use Remove-PSDrive or dismount in Disk Management" -ForegroundColor Yellow'
        'lsof' = 'Get-Process | ForEach-Object { Get-NetTCPConnection -OwningProcess $_.Id -ErrorAction SilentlyContinue } | Format-Table'
        'strace' = 'Write-Host "strace: Use Process Monitor (ProcMon) on Windows" -ForegroundColor Yellow'
        'tcpdump' = 'Write-Host "tcpdump: Use Wireshark or netsh trace on Windows" -ForegroundColor Yellow'
    }
    ...
    foreach ($cmdName in $linuxCommands.Keys) {
        $cmdBody = $linuxCommands[$cmdName]
        
        # Skip commands with spaces (invalid function names)
        if ($cmdName -match '\s') {
            continue
        }
        
        # Check if function already exists
        $functionPattern = "function\s+$cmdName\s*\{"
        if ($profileContent -match $functionPattern) {
            $skippedCommands += $cmdName
            $skippedCount++
            continue
        }

        # Check if alias exists and disable it
        if (Test-Path alias:$cmdName) {
            Remove-Item alias:$cmdName -Force -ErrorAction SilentlyContinue
            $removeAliasBlock = @"
if (Test-Path alias:$cmdName) { Remove-Item alias:$cmdName -Force }
"@
            Add-Content -Path $PROFILE -Value "`n$removeAliasBlock"
        }
...
"@
        Add-Content -Path $PROFILE -Value "`n$functionBlock"
        $addedCommands += $cmdName
        $addedCount++
    }

    Write-Host ""
    if ($addedCount -gt 0) {
        Write-Host "Successfully added $addedCount Linux command(s):" -ForegroundColor Green
        foreach ($cmd in $addedCommands) {
    ...
    if ($skippedCount -gt 0) {
        Write-Host ""
        Write-Host "Skipped $skippedCount command(s) (already exist):" -ForegroundColor Yellow
        foreach ($cmd in $skippedCommands) {
    ...
    try {
        . $PROFILE
        Write-Host "Profile reloaded. Linux commands are now available!" -ForegroundColor Green
    } catch {
        Write-Host "Warning: Profile reloaded with errors. Some commands may not work until you restart PowerShell." -ForegroundColor Yellow
    }
}
