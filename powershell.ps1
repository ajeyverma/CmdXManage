...
function Restore-FromGitHub {
    $url = "https://raw.githubusercontent.com/ajeyverma/CmdXManage/main/powershell.ps1" # Replace
    $localPath = $PSCommandPath
    try {
        Invoke-WebRequest -Uri $url -OutFile $localPath -ErrorAction Stop
        Write-Host "The script has been restored from GitHub." -ForegroundColor Green
        Write-Host "Please restart the script."
        return
    } catch {
        Write-Host "ERROR: Could not restore the script from GitHub." -ForegroundColor Red
        Write-Host "Check your internet connection and try again." -ForegroundColor Yellow
        return
    }
}

function Ensure-Script-Integrity {
    $expectedHash = "54f98b1ab83fbbef0599fae3ae600d568911eed87dc69e6f7b2b534e3f16f06d" # Replace with real hash (calculated without this line)
    $scriptPath = $PSCommandPath
    Write-Host "$scriptpath"
    # Calculate hash ignoring the $expectedHash line
    $sccontent = (Get-Content $scriptpath | Where-Object { $_ -notmatch '^\s*\$expectedHash\s*=' }) -join "`n"
    $scbytes   = [System.Text.Encoding]::UTF8.GetBytes($sccontent)
    $scsha     = [System.Security.Cryptography.SHA256]::Create()
    $schashBytes = $scsha.ComputeHash($scbytes)
    $schash    = ($schashBytes | ForEach-Object { $_.ToString("x2") }) -join ''
    Write-Host "SHA256 (excluding `$expectedHash line):" $schash

    if ($schash -ne $expectedHash) {
        Write-Host "Script has been changed. Restoring original version from GitHub..." -ForegroundColor Red
...
#________________________________________________________________________________________________________
function Remove-Command { # REMOVE COMMAND
